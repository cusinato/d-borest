...
label_train <- fread("gender_age_train.csv",
                     colClasses=c("character","character",
                                  "integer","character"))
label_test <- fread("gender_age_test.csv",
...
brand <- fread("phone_brand_device_model.csv",
...
event_app <- fread("app_events.csv",
...
### Agrega variáveis latlon - inicio
agrega_campos1 <- function(orig, dados){
  load(dados)
  #quero agregar os device_id faltantes e ordenar a tabela da mesma forma que a de treinamento
  ids <- data.table(device_id = rownames(orig))
  data2 <- merge(ids, m1, by='device_id', all.x=TRUE)
  data2[is.na(data2)] <- 0 # imputei com zeros pq sou preguiçoso (e pra me aproveitar de dgCMatrix)
  did <- data2$device_id
  data2[, device_id := NULL]
  data2 <- as.matrix(data2)
  rownames(data2) <- did
  cBind(orig, data2)
}
agrega_campos2 <- function(orig, dados){
  load(dados)
  #quero agregar os device_id faltantes e ordenar a tabela da mesma forma que a de treinamento
  ids <- data.table(device_id = rownames(orig))
  data2 <- merge(ids, m2, by='device_id', all.x=TRUE)
  data2[is.na(data2)] <- 0 # imputei com zeros pq sou preguiçoso (e pra me aproveitar de dgCMatrix)
  did <- data2$device_id
  data2[, device_id := NULL]
  data2 <- as.matrix(data2)
...
agrega_campos3 <- function(orig, dados){
  load(dados)
  #quero agregar os device_id faltantes e ordenar a tabela da mesma forma que a de treinamento
  ids <- data.table(device_id = rownames(orig))
  data2 <- merge(ids, m3, by='device_id', all.x=TRUE)
  data2[is.na(data2)] <- 0 # imputei com zeros pq sou preguiçoso (e pra me aproveitar de dgCMatrix)
  did <- data2$device_id
  data2[, device_id := NULL]
  data2 <- as.matrix(data2)
...
arq <- grep('min_max_med_[0-9]+a[0-9]+\\.RData', list.files(), value = T)
de  <- as.numeric(gsub('min_max_med_([0-9]+)a([0-9]+)\\.RData', '\\1', arq))
ate <- as.numeric(gsub('min_max_med_([0-9]+)a([0-9]+)\\.RData', '\\2', arq))
arq <- arq[order(de)]
ate <- ate[order(de)]
de <- de[order(de)]

for (i in arq){
  #   train_data <- agrega_campos(train_data, i)
  test_data <- agrega_campos1(test_data, i); gc()
}
for (i in arq){
  #   train_data <- agrega_campos(train_data, i)
  test_data <- agrega_campos2(test_data, i); gc()
}
for (i in arq){
  #   train_data <- agrega_campos(train_data, i)
  test_data <- agrega_campos3(test_data, i); gc()
}
### Agrega variáveis latlon - final
dtrain <- xgb.DMatrix(testeBind,label=train_label,missing=NA)
